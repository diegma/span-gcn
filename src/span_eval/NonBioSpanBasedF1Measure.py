...
import torch
from models.custom_allennlp.elmo_dependencies.util import ones_like, get_lengths_from_binary_sequence_mask
from data_preprocess.framenet.ontology_reader import FrameOntology
from span_eval.metric import Metric
...
class NonBioSpanBasedF1Measure(Metric):
    ...
    def __init__(self,
        ...
        self.num_classes = len(vocabulary) #vocabulary.get_vocab_size(tag_namespace)
    ...
    def __call__(self,
        ...
        if (gold_labels >= self.num_classes).any():
            raise IOError("A gold label passed to NonBioSpanBasedF1Measure contains an "
        ...
        argmax_predictions = torch.Tensor(predictions).long()
        ...
        for i in range(batch_size):
            ...
            predicted_spans = self._extract_spans_from_BIO(predicted_sequence[:length].tolist(), merge=True)
            gold_spans = self._extract_spans_from_BIO(gold_sequence[:length].tolist(), merge=True)
    ...
    def _extract_spans_from_BIO(self, tag_matrix: List[List[int]], merge: bool = False) -> Set[Tuple[int, int, str]]:
        """
        Given an integer tag sequence corresponding to BIO tags, extracts spans.
        Spans are inclusive and can be of zero length, representing a single word span.
        Ill-formed spans are also included (i.e those which do not start with a "B-LABEL"),
        ...
        inside = ''
        for i, lab_idx in enumerate(tag_matrix):
            if lab_idx != 0:
                if self._label_vocabulary[lab_idx][0] == 'B':
                    if inside == self._label_vocabulary[lab_idx][2:]:
                        spans.add((span_start, i-1, inside))
                    else:
                        span_start = i
                        inside = self._label_vocabulary[lab_idx][2:]
                elif self._label_vocabulary[lab_idx][0] == 'I':
                    if inside != self._label_vocabulary[lab_idx][2:]:
                        if inside == '':
                            span_start = i
                            inside = self._label_vocabulary[lab_idx][2:]
