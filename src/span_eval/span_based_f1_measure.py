...
from models.custom_allennlp.elmo_dependencies.util import get_lengths_from_binary_sequence_mask
from models.custom_allennlp.elmo_dependencies.vocabulary import Vocabulary
from span_eval.metric import Metric

from span_eval.span_utils import (
        bio_tags_to_spans,
        bioul_tags_to_spans,
        iob1_tags_to_spans,
        bmes_tags_to_spans,
...
class SpanBasedF1Measure(Metric):
    ...
    def __init__(self,
        ...
        if label_encoding:
            if label_encoding not in ["BIO", "IOB1", "BIOUL", "BMES"]:
                raise IOError("Unknown label encoding - expected 'BIO', 'IOB1', 'BIOUL', 'BMES'.")
    ...
    def __call__(self,
        ...
        gold_labels, mask, prediction_map = self.unwrap_to_tensors(gold_labels,mask, prediction_map)

        # num_classes = predictions.size(-1)
        # if (gold_labels >= num_classes).any():
        #     raise IOError("A gold label passed to SpanBasedF1Measure contains an "
        ...
        argmax_predictions = torch.Tensor(argmax_predictions).view(1, -1).long()
