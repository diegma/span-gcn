...
def infer_and_cast(value: Any):
    ...
    if isinstance(value, (int, float, bool)):
        ...
        raise ValueError("cannot infer type of "+value+"")
    ...
    for compound_key, value in flat_dict.items():
        ...
        for key in parts[:-1]:
            ...
            if key not in curr_dict:
                ...
                raise IOError("flattened dictionary is invalid")
...
class Params(MutableMapping):
    ...
    def add_file_to_archive(self, name: str) -> None:
        ...
        if not self.loading_from_archive:
            self.files_to_archive[str(self.history)+""+str(name)] = cached_path(self.get(name))
    ...
    def pop(self, key: str, default: Any = DEFAULT) -> Any:
        ...
        if default is self.DEFAULT:
            try:
                ...
                raise IOError("key \"{}\" is required at location \"{}\"".format(key, self.history))
        else:
    ...
    def assert_empty(self, class_name: str):
        ...
        """
        if self.params:
            raise IOError("Extra parameters passed to {}: {}".format(class_name, self.params))
