...
from models.custom_allennlp.elmo_dependencies.from_params import FromParams
...
class Registrable(FromParams):
    ...
    def by_name(cls: Type[T], name: str) -> Type[T]:
        logger.debug("instantiating registered subclass "+name+" of "+cls+"")
        if name not in Registrable._registry[cls]:
            raise IOError("%s is not a registered name for %s" % (name, cls.__name__))
