...
from models.custom_allennlp.elmo_dependencies.token_indexer import TokenIndexer, TokenType
...
from models.custom_allennlp.elmo_dependencies.field import DataArray, Field
...
class TextField(SequenceField[Dict[str, torch.Tensor]]):
    ...
    def __init__(self, tokens: List[Token], token_indexers: Dict[str, TokenIndexer]) -> None:
        ...
        if not all([isinstance(x, (Token, SpacyToken)) for x in tokens]):
            raise IOError("TextFields must be passed Tokens. "
    ...
    def get_padding_lengths(self) -> Dict[str, int]:
        ...
        if self._indexed_tokens is None:
            raise IOError("You must call .index(vocabulary) on a "
        ...
        for indexer_name, token_list in self._indexed_tokens.items():
            padding_lengths[str(indexer_name)+"_length"] = len(token_list)
    ...
    def as_tensor(self, padding_lengths: Dict[str, int]) -> Dict[str, torch.Tensor]:
        tensors = {}
        for indexer_name, indexer in self._token_indexers.items():
            desired_num_tokens = {indexed_tokens_key: padding_lengths[str(indexed_tokens_key)+"_length"]
                                  for indexed_tokens_key in self._indexer_name_to_indexed_token[indexer_name]}
