...
def sort_batch_by_length(tensor: torch.Tensor, sequence_lengths: torch.Tensor):
    ...
    if not isinstance(tensor, torch.Tensor) or not isinstance(sequence_lengths, torch.Tensor):
        raise IOError("Both the tensor and sequence lengths must be torch.Tensors.")
...
def masked_flip(padded_sequence: torch.Tensor,
    ...
    # , \
    #     "sequence_lengths length $"+str(len(sequence_lengths)+" does not match batch size $"+padded_sequence.size(0)+""
...
def replace_masked_values(tensor: torch.Tensor, mask: torch.Tensor, replace_with: float) -> torch.Tensor:
    ...
    if tensor.dim() != mask.dim():
        raise IOError("tensor.dim() (%d) != mask.dim() (%d)" % (tensor.dim(), mask.dim()))
...
def combine_tensors(combination: str, tensors: List[torch.Tensor]) -> torch.Tensor:
    ...
    """
    if len(tensors) > 9:
        raise IOError("Double-digit tensor lists not currently supported")
...
def _rindex(sequence: Sequence[T], obj: T) -> int:
    ...
    raise ValueError("Unable to find "+obj+" in sequence "+str(sequence)+".")
...
def _get_combination(combination: str, tensors: List[torch.Tensor]) -> torch.Tensor:
    if combination.isdigit():
        ...
        if len(combination) != 3:
            raise IOError("Invalid combination: " + combination)
        ...
        if operation == '*':
            ...
            raise IOError("Invalid operation: " + operation)
...
def _get_combination_dim(combination: str, tensor_dims: List[int]) -> int:
    if combination.isdigit():
        ...
        if first_tensor_dim != second_tensor_dim:
            raise IOError("Tensor dims must match for operation \"{}\"".format(operation))
...
def flattened_index_select(target: torch.Tensor,
    ...
    """
    if indices.dim() != 2:
        raise IOError("Indices passed to flattened_index_select had shape {} but "
